  ...
  const [project] = useState<Data[]>([
    {
      ...
      experience: [
        ...
        'pnpm 모노레포 웹·API 패키지 분리 및 빌드·배포 체계 정립 → 개발·운영 작업 흐름 표준화',
    ...
    },
    {
      ...
      experience: [
        ...
        'NestJS + TypeORM + MySQL + Redis 기반 API 연동 → 정산·리포트·대시보드 데이터 안정적 제공',
        '영업/유지보수 데이터 기반 정산관리(청구·입금·지연·상태) 기능 구현 → 운영 정산 업무 효율화',
        '정산 목록 조회·필터·정렬·엑셀 다운로드 UX 및 서버 조회 로직 최적화 → 대용량 데이터 환경 대응',
        '영업·유지보수 도메인 분리 설계 및 공통 조회 패턴 적용 → 필터/정렬/표시 규칙 일관성 확보',
        '백엔드 join + 파생 필드 표준화 → 정산/리포트/대시보드 화면의 프론트 중복 로직 최소화',
        'sales·maintenance 응답 스키마 분리/확장 → 필드 의미 혼재 방지 및 장기 유지보수성 확보',
      ],
      people: '개발 3',
      stack: [
        { name: 'Vue 3', icon: <SiVuedotjs /> },
