  ...
  const [project] = useState<Data[]>([
    {
      ...
      date: '2026.05 - 현재',
      experience: [
        'Vue 3 + TypeScript + Vite 기반 웹 애플리케이션 구현 → 인사·문서·전자계약·전자결재 업무 흐름 통합',
        'NestJS + PostgreSQL 기반 API·도메인 설계 → 인사/문서/결재 데이터 일관성 및 확장성 확보',
        '모노레포 웹·API 패키지 분리 및 빌드·배포 체계 정립 → 개발·운영 작업 흐름 표준화',
        '리치 에디터·뷰어 기반 계약/사규/공지·템플릿 문서 UI 구축 → 작성·검토·미리보기 품질 향상',
        '라이트/다크·OS 테마 연동 및 문서 UI 스타일 공통화 → 다양한 환경에서 안정적인 문서 표시',
    ...
    },
    {
      ...
      date: '2025.12 - 현재',
      experience: [
        'Vue 3 + TypeScript + Vite 기반 Web Admin 구현 → 영업·유지보수·정산 업무 통합 관리',
        'NestJS + MySQL + Redis 기반 API 연동 → 정산·리포트·대시보드 데이터 안정적 제공',
        '영업/유지보수 데이터 기반 정산관리(청구·입금·지연·상태) 기능 구현 → 운영 정산 업무 효율화',
        '정산 목록 조회·필터·정렬·엑셀 다운로드 UX 및 성능 최적화 → 대용량 데이터 환경 대응',
        '영업·유지보수 업무 유형별 정산 체계 설계 → 조회·관리 흐름 일관성 확보',
        '정산·리포트·대시보드 데이터 표준화 → 화면 간 표시 일관성 및 운영 신뢰도 향상',
      ],
      people: '개발 3',
      stack: [
        { name: 'Vue 3', icon: <SiVuedotjs /> },
